...
func TestCatalogAuthorizer_CanDeleteAlbum(t *testing.T) {
	owner1 := ownermodel.Owner("owner-1")
	owner2 := ownermodel.Owner("owner-2")
	userOfOwner1 := usermodel.CurrentUser{UserId: "user-1", Owner: &owner1}
	...
	tests := []struct {
		...
		{
			name: "allows deletion when CurrentUser.Owner is defined and equals AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "denies deletion when CurrentUser.Owner is not defined and user has no MainOwner permissions",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "denies deletion when CurrentUser.Owner is defined but not equals AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "allows deletion when CurrentUser.Owner is not defined and user is MainOwner of AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{
					Scopes: []*aclcore.Scope{
		...
		{
			name: "denies deletion when CurrentUser.Owner is not defined and user is MainOwner of different owners",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{
					Scopes: []*aclcore.Scope{
						{Type: aclcore.MainOwnerScope, GrantedTo: userNoOwner.UserId, ResourceOwner: owner2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := a.CanDeleteAlbum(tt.args.ctx, tt.args.user, tt.args.albumId)
			tt.wantErr(t, err, "CanDeleteAlbum(%v, %v, %v)", tt.args.ctx, tt.args.user, tt.args.albumId)
		})
	}
}
...
func TestCatalogAuthorizer_CanRenameAlbum(t *testing.T) {
	owner1 := ownermodel.Owner("owner-1")
	owner2 := ownermodel.Owner("owner-2")
	userOfOwner1 := usermodel.CurrentUser{UserId: "user-1", Owner: &owner1}
	userOfOwner2 := usermodel.CurrentUser{UserId: "user-2", Owner: &owner2}
	...
	tests := []struct {
		...
		{
			name: "allows renaming when CurrentUser.Owner is defined and equals AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "denies renaming when CurrentUser.Owner is not defined and user has no MainOwner permissions",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "denies renaming when CurrentUser.Owner is defined but not equals AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{},
			},
			args: args{
		...
		{
			name: "allows renaming when CurrentUser.Owner is not defined and user is MainOwner of AlbumId.Owner",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{
					Scopes: []*aclcore.Scope{
						{Type: aclcore.MainOwnerScope, GrantedTo: userNoOwner.UserId, ResourceOwner: owner2},
		...
		{
			name: "denies renaming when CurrentUser.Owner is not defined and user is MainOwner of different owners",
			fields: fields{
				HasPermissionPort: &aclcore.ScopeReadRepositoryInMemory{
					Scopes: []*aclcore.Scope{
						{Type: aclcore.MainOwnerScope, GrantedTo: userNoOwner.UserId, ResourceOwner: owner2},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := a.CanRenameAlbum(tt.args.ctx, tt.args.user, tt.args.albumId)
			tt.wantErr(t, err, "CanRenameAlbum(%v, %v, %v)", tt.args.ctx, tt.args.user, tt.args.albumId)
		})
	}
}
