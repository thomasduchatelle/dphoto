...
func Test_repository_ListOwnerScopes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "it should not find any scope when user has no grant of that type",
			args:    args{"pepperpotts@stark.com", []accesscontrol.ScopeType{accesscontrol.MediaVisitorScope}},
			wantErr: assert.NoError,
		},
		{
			name:    "it should find to whom a user shares its content",
			args:    args{"pepperpotts@stark.com", []accesscontrol.ScopeType{accesscontrol.AlbumVisitorScope}},
			wantErr: assert.NoError,
			want: []*accesscontrol.Scope{
				{
					Type:          accesscontrol.AlbumVisitorScope,
