...
2. Prioritise exact semantic over correct syntax: syntax error can be found by the compiler and fixed easily, semantic error could void a test without being
   noticed.
3. Use Test Driven Development methods: create new tests instead of updating existing ones (unless necessary to make it pass).
...
* `web-nextjs` - root of the NextJS project using the App Router convention
    * `package.json`
    * `app/` - exposed paths using NextJS App Router.
        * `(authenticated)` - all pages sharing the same layout, for authenticated users.
            * `albums/[owner]/[albumId]/` - path for the media listing
                * `_components` - directory containing the components used once, and only on this page: modals, listings, ...
                    * `CreateAlbumModals/` - component are ALWAYS in a directory, naming convention is to match the main component (PascalCase for react
                      component, camelCase for hooks, ...)
                        * `index.tsx` - components are ALWAYS exported from an index file.
                        * `CreateAlbumModal.stories.tsx` - visual tests are alongside the component, in a file that has the name of the component.
            * `layout.tsx` - common layout for all pages where user is authenticated
            * `error.tsx`
            * `loading.tsx`
    * `domains` - pure TypeScript business logic, following the Domain Driven Design and Hexagonal Architecture concepts, no NextJS or React dependencies.
        * `catalog/index.ts` - export all the members of the "catalog" domain, used in the pages, and on the components.
    * `components/` - all other components: used in several places or in the layouts. Each component must have its own folder and be exported from `index.tsx`
      file.
    * `libs/` - technical utilities, each on its own folder.
        * `security/` - integrates the OIDC library
        * `requests/` - utilities to handle basepath and forwarded header on routes requiring it
...
1. **TDD principle**: **do not implement any behaviour on the code which is not required by a test**. Keep the code minimal and simple.
...
2. **Robust Tests**: tests must be robust to refactoring
    * actions and selectors are **tested together**: initiate the state -> create action -> execute the reducer -> run the selector -> assert the returned
      selection
    * use or extends pre-defined constants from `web-nextjs/domains/_tests/test-helper-state.ts`
    * use fake implementations, not mocks, so changing the method signature of the Ports does not affect the tests.
...
1. **UI Components (Views)**: purely presentational. They receive all necessary data via props and trigger thunks in response to user interactions.
   They do not contain any business logic or direct state manipulation.

2. **Thunks**: stateless and framework-agnostic functions that encapsulate all business logic. They are triggered by UI events, interact with adapters (ports)
   for external operations (like API calls), and dispatch actions to update the application state.
