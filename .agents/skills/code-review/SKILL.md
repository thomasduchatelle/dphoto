...
2. **risk of regression** - existing features and behaviour will fail using this new code. Look after what's not obvious and covered with test, includes misconfiguration, backward compatibility (or impossibility to rollback), ...
...
4. **risk of poor maintainability** - architecture and standard has been violated, testing strategy hasn't been respected, and that will affect future development. Look for responsibility leakage, poor boundaries, ... You're not nitpicking here, it's confirmed serious offences.
