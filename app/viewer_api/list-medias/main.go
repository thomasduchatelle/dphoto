...
func Handler(request events.APIGatewayProxyRequest) (common.Response, error) {
	...
	for i, media := range medias.Content {
		restKey := fmt.Sprintf("/v1/owners/%s/medias/%s/%d", owner, media.Signature.SignatureSha256, media.Signature.SignatureSize)
		...
		resp[i] = Media{
			...
			Source: fmt.Sprintf("%s %s", media.Details.Make, media.Details.Model),
		}
	}
